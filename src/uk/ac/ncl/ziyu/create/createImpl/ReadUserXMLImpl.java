...
public class ReadUserXMLImpl {
	...
	private static boolean isBotUser(String user) {
				
		String[] knownBots = {"JustAGal" };
		
		System.out.println("is user "+user+"  a bot?");
		
		for (String botUser: knownBots)  {
			if (botUser.equals(user))  {
				System.out.println("known bot found: "+botUser);
				return true;
			} else {
				System.out.println(botUser+" not the same as "+user);
			}
		}
		
		
		if (user.contains("Bot") || user.contains("bot")) {
			System.out.println("probable new bot found: "+user);
			return true;
		}
